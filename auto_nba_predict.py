...
st.set_page_config(page_title="NBA 數據專家 v7.6", layout="wide")
...
# --- 3. 數據核心 (修正 NameError 問題) ---
@st.cache_data(ttl=3600)
def load_all_data_v76():
...
clf, reg, gf, ps_full, feats, maps, player_stats_db, last_update = load_all_data_v76()
...
# --- 5. 介面呈現 ---
st.title("🏀 NBA 數據專家 v7.6 (整合盤口價值分析)")
...
for i, tab in enumerate(tabs):
    with tab:
        ...
        if game_options:
            ...
            if abs(diff) > 5:
                st.success(f"🔥 AI 發現價值！比莊家多出 {abs(diff):.1f}% 信心看好 {'主隊' if diff > 0 else '客隊'}")
            else:
                st.warning("⚖️ AI 與盤口看法接近，無顯著投注價值。")
...
st.sidebar.info(f"🚑 ESPN 傷病庫已抓取 {sum(len(v) for v in injury_report.values())} 位球員")
