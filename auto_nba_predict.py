...
# --- 1. AI 核心設定 (使用新版 google-genai SDK) ---
@st.cache_resource
def init_ai_v8():
    if "GEMINI_API_KEY" in st.secrets:
        try:
            ...
            # 自動偵測模型邏輯 (新版 SDK 語法)
            # 優先級：gemini-2.0-flash (2026主流) > gemini-1.5-flash
...
client, model_id = init_ai_v8()
AI_READY = True if client else False
...
st.set_page_config(page_title="NBA AI 數據專家 v6.8", layout="wide")
st.title("🏀 NBA 終極智慧預測系統 v6.8")
if AI_READY:
    st.caption(f"🚀 已升級至新版 SDK (模型: {model_id})")

# --- 2. 賽程獲取 (保持每次更動：V2/V3 自動校正) ---
...
def get_full_analysis(raw_list, clf, reg, gf, ps, feats):
    ...
    if ai_input and AI_READY:
        with st.spinner("🧠 使用新版 SDK 進行 AI 深度分析..."):
            prompt = f"你是 NBA 數據專家。請為以下比賽寫 180 字以上分析，包含整數分差與勝率。回傳 JSON: {{'場次ID': '內容'}}。\n{ai_input}"
            try:
                # 新版 SDK 的生成方式 (使用 Client)
                ...
                for gid in results: results[gid]['report'] = f"AI 調用異常: {str(e)}"
    return results
