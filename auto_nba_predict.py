...
st.set_page_config(page_title="NBA 數據專家 v7.0", layout="wide")
st.title("🏀 NBA 數據專家 v7.0 (全數據聯動預測版)")
...
def fetch_safe_df(endpoint_class, **kwargs):
    try:
        ...
        # 彈性匹配 ID 欄位，解決戰術數據找不到 TEAM_ID 的問題
...
def load_all_data_v70():
    ...
    # [D] 模型核心：全指標特徵工程 (Feat Engineering)
    ...
    # 注入全部 16 項視覺化指標到模型中
    gf['F_PTS'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'base', 'PTS', 110))
    gf['F_REB'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'base', 'REB', 44))
    gf['F_AST'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'base', 'AST', 25))
    gf['F_ORTG'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'adv', 'OFF_RATING', 110))
    gf['F_DRTG'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'adv', 'DEF_RATING', 110))
    gf['F_PACE'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'adv', 'PACE', 99))
    gf['F_DEFL'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'hustle', 'DEFLECTIONS', 15))
    gf['F_CONT'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'hustle', 'CONTESTED_SHOTS', 40))
    gf['F_DIST'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'spd', 'DIST_MILES', 18))
    gf['F_SPD'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'spd', 'AVG_SPEED', 4.4))
    gf['F_PASS'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'pass', 'PASSES_MADE', 280))
    gf['F_TRANS'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'trans', 'PPP', 1.1))
    gf['F_ISO'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'iso', 'PPP', 0.9))
    gf['F_RIM'] = gf['TEAM_ID'].apply(lambda x: get_v(x, 'rim', 'D_FG_PCT', 0.6))
...
clf, reg, gf, ps_full, feats, maps, last_update = load_all_data_v70()
    ...
    with tab:
        ...
        if sb.empty: st.info("📅 今日無賽程")
            ...
            if games:
                ...
                c3.metric("AI 全數據預測贏家", res['winner'], f"預計分差: {res['diff']} 分")
                ...
                st.table(pd.DataFrame({
                    "指標": ["撥球破壞 (Deflections)", "干擾投籃 (Contested)", "場均里程", "平均速度", "場均傳球數"],
                ...
                # --- 表格 3：戰術與護框 (徹底修復 0 值) ---
                st.subheader("⚔️ 3. 戰術類別與護框效率 (AI 建模特徵)")
                st.table(pd.DataFrame({
                    "指標": ["轉換進攻效率 (PPP)", "單打得分效率 (PPP)", "籃框護框命中率 (D-FG%)"],
