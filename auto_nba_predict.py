...
st.set_page_config(page_title="NBA 專家 v12.3", layout="wide")

# --- 2. 修正後的 CBS 抓取函數 (解決 KeyError) ---
@st.cache_data(ttl=600)
def get_cbs_injuries_fixed():
    ...
    try:
        ...
        # 修正後的解析邏輯
        containers = soup.find_all('div', class_='TableBase')
        for container in containers:
            # 抓取隊名
            title_div = container.find('span', class_='TeamLogoNameLockup-name')
            ...
            for r in rows:
                ...
                if len(cols) >= 5:
                    all_data.append({
                        ...
                        '狀態': cols[4].get_text(strip=True),
                        '球隊': t_abbr,
                        '說明': cols[3].get_text(strip=True)
                    })
    except: pass
    
    # 關鍵防錯：確保回傳正確欄位的 DataFrame
...
injury_df = get_cbs_injuries_fixed()

# --- 3. UI 顯示 (文字標籤化) ---
st.title("🏀 NBA 數據專家 v12.3 (CBS 結構修復版)")
...
for i, tab in enumerate(tabs):
    with tab:
        ...
        for idx, row in sb.iterrows():
            ...
            g_key = f"v123_{dates[i].strftime('%Y%m%d')}_{a_abbr}_{h_abbr}"
