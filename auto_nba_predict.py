...
st.title("🏀 NBA 數據專家 v7.1 (模型除錯 & 全維度分析)")
    ...
    with tab:
        ...
        if sb.empty: st.info("📅 今日無賽程")
            ...
            if games:
                ...
                c3.metric("AI 預測贏家", res['winner'], f"預計分差: {res['diff']} 分")
                ...
                st.table(pd.DataFrame({
                    "指標項目": ["場均得分", "場均籃板", "場均助攻", "進攻效率", "防守效率"],
                    res['h_name']: [f"{gm('base',res['h_id'],'PTS'):.1f} 分", f"{gm('base',res['h_id'],'REB'):.1f} 個", f"{gm('base',res['h_id'],'AST'):.1f} 次", f"{gm('adv',res['h_id'],'OFF_RATING')} pts/100", f"{gm('adv',res['h_id'],'DEF_RATING')} pts/100"],
                    res['a_name']: [f"{gm('base',res['a_id'],'PTS'):.1f} 分", f"{gm('base',res['a_id'],'REB'):.1f} 個", f"{gm('base',res['a_id'],'AST'):.1f} 次", f"{gm('adv',res['a_id'],'OFF_RATING')} pts/100", f"{gm('adv',res['a_id'],'DEF_RATING')} pts/100"]
                }))

                # 表格 2：體能
                st.subheader("🏃‍♂️ 2. 體能與積極度 (場均)")
                st.table(pd.DataFrame({
                    "指標": ["撥球破壞", "跑動里程", "平均速度", "場均傳球"],
                    res['h_name']: [f"{gm('hustle',res['h_id'],'DEFLECTIONS'):.1f} 次", f"{gm('spd',res['h_id'],'DIST_MILES'):.2f} mi", f"{gm('spd',res['h_id'],'AVG_SPEED'):.2f} mph", f"{gm('pass',res['h_id'],'PASSES_MADE'):.1f} 次"],
                    res['a_name']: [f"{gm('hustle',res['a_id'],'DEFLECTIONS'):.1f} 次", f"{gm('spd',res['a_id'],'DIST_MILES'):.2f} mi", f"{gm('spd',res['a_id'],'AVG_SPEED'):.2f} mph", f"{gm('pass',res['a_id'],'PASSES_MADE'):.1f} 次"]
                }))

                # 表格 3：戰術
                st.subheader("⚔️ 3. 戰術類別與護框")
                st.table(pd.DataFrame({
                    "指標": ["轉換進攻 PPP", "單打 PPP", "護框命中率 %"],
