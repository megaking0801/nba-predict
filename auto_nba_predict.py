...
def run_prediction(games, clf, reg, all_games_raw, player_stats, features_list):
    ...
    for g_id, res in results.items():
        ...
        final_results[g_id]['summary_report'] = ai_book.get(g_id, f"【數據快評】{TEAM_NAME_CH.get(res['winner_abbr'])} 因近期場均得分與勝率較高，模型看好能拿下比賽。")
...
for i, tab in enumerate(tabs):
    with tab:
        ...
        if not games: st.info("暫無賽程數據"); continue
