...
st.set_page_config(page_title="NBA 專家 v12.6", layout="wide")
...
def get_espn_injuries_v2():
    ...
    try:
        ...
        if not sections:
            # 備援：嘗試搜尋另一種 ESPN 常見標籤
            sections = soup.find_all('span', class_='Table__Title')
        ...
        for section in sections:
            ...
            if table:
                rows = table.find_all('tr')[1:] # 略過標題行
                for r in rows:
                    cols = r.find_all('td')
                    if len(cols) >= 3:
                        all_inj.append({
...
# --- 3. UI 顯示邏輯 ---
st.title("🏀 NBA 數據專家 v12.6 (傷病+受讓分完整版)")
    ...
    with tab:
        ...
        for idx, row in sb.iterrows():
            ...
            def build_team_package(tid, abbr):
                ...
                # 排除 Out, Doubtful, Day-To-Day (後者視情況調整，SGA 常標記為 D2D)
            ...
            g_key = f"v126_{dates[i].strftime('%Y%m%d')}_{a_abbr}_{h_abbr}"
            ...
            with cols[idx % 3]:
                with st.container(border=True):
                    ...
                    if show_odds:
                        ...
                        spread = st.number_input(f"🚩 [主隊] 受讓分 (負數代表讓分)", value=0.0, step=0.5, key=f"sp_{g_key}")
